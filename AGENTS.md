...
#### 整体布局（参考Navicat Premium）

```
┌─────────────────────────────────────────────────────────────┐
│ 菜单栏                                                        │
├─────────────────────────────────────────────────────────────┤
│ 工具栏: [连接] [查询] [表] [视图] [函数] [用户] [其它] ...    │
├──────────┬──────────────────────────────────────────────────┤
│ 侧边栏   │ 内容区                                            │
│          │ ┌──────────────────────────────────────────────┐ │
│ 我的连接  │ │ 标签页: [查询1] [表名] [设计: 表名] [+]      │ │
│          │ ├──────────────────────────────────────────────┤ │
│ ▼ mysql  │ │                                              │ │
│   ├─ db1 │ │         SQL编辑器 / 数据网格 / 表设计         │ │
│   │ ├─ t1│ │                                              │ │
...
- **左侧功能按钮组**: 连接、查询、表、视图、函数、用户、其它
- **右侧操作按钮组**: 执行、停止、刷新、导入/导出、搜索
- **按钮样式**: 图标+文字，激活态高亮

#### 侧边栏设计

- **顶部标题**: "我的连接"
- **连接树**: 层级结构（连接 → 数据库 → 表/视图/存储过程）
- **底部搜索框**: 快速定位连接
